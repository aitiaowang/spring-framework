...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			//不要在这里初始化FactoryBeans:我们需要离开所有的常规bean，不初始化，让bean工厂后处理器应用到它们!
			//在实现的BeanDefinitionRegistryPostProcessors之间分离PriorityOrdered、Ordered和其余的。
			List<BeanDefinitionRegistryPostProcessor> currentRegistryProcessors = new ArrayList<>();

			// First, invoke the BeanDefinitionRegistryPostProcessors that implement PriorityOrdered.
			// 首先，调用实现PriorityOrdered的BeanDefinitionRegistryPostProcessors。(配置注册的后处理器)
		...
		//将实现PriorityOrdered，Ordered的BeanFactoryPostProcessor和其余的实现分开。
		...
		// 首先，调用实现PriorityOrdered的BeanFactoryPostProcessors。调用之前，先按照优先级进行排序
		...
		// 最后，调用所有其他BeanFactoryPostProcessors。(无序，直接调用)
	...
	public static void registerBeanPostProcessors(
		...
		//注册BeanPostProcessorChecker，当bean是在BeanPostProcessor实例化过程中创建的，即当一个bean不能被所有的BeanPostProcessors处理。
		/**
		...
		// 实现PriorityOrdered，Ordered的BeanPostProcessor与其余的分开(按优先值，优先执行)
	 ...
	 * <p>
	 * 在过程中创建bean时记录一条信息消息的BeanPostProcessorBeanPostProcessor实例化，
	 * 即当一个bean不符合条件时被所有BeanPostProcessors处理。
	 */
