 ...
 * interface.
 * <p>
 * 由大多数bean工厂实现的配置接口。提供了配置一个bean工厂的设施，除了bean工厂
 ...
 * <p>
 * 此bean工厂接口不适合在常规应用程序中使用代码：
 * 坚持使用{@link org.springframework.beans.factory.BeanFactory}或{@link org.springframework.beans.factory.ListableBeanFactory}来满足典型的需求。
...
public interface ConfigurableBeanFactory extends HierarchicalBeanFactory, SingletonBeanRegistry {
	 ...
	 * <p>
	 * 注意：此处提交的后处理器将按照*注册的顺序应用；通过实现{@link org.springframework.core.Ordered}接口表示的任何排序语义都将被忽略。
	 * 注意自动检测到的后处理器（例如，作为ApplicationContext中的bean）将始终以编程方式注册后的处理器。
	 *
	 ...
	 * 该bean是否为FactoryBean({@code false}表示该bean存在，但不是FactoryBean)
	 ...
	 * and logged instead of propagated to the caller of this method.
	 * <p>
	 * 销毁过程中出现的任何异常都应被捕获*并记录下来，而不是传播给此方法的调用者
	 */
