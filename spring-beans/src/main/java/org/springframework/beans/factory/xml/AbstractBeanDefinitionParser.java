...
public abstract class AbstractBeanDefinitionParser implements BeanDefinitionParser {
	 ...
	 * <p>
	 * 子类可以重写此方法，以控制所提供的{@link BeanDefinitionHolder bean}是否实际上已注册，或注册更多的bean。
	 *
	 ...
	 * <p>
	 * 仅当{@code isNested}参数为{@code false}时，默认实现才使用提供的{@link BeanDefinitionRegistry registry}
	 * 注册提供的{@link BeanDefinitionHolder bean}，因为通常不希望将内部bean用作注册为顶级bean。
	 *
	 * @param definition the bean definition to be registered
	 ...
	 * <p>
	 * 确定在解析bean定义之后，是否应该向该解析器触发{@link org.springframework.beans.factory.parsing.BeanComponentDefinition}事件。
	 *
	 ...
	 * <p>
	 * 默认情况下，此实现返回{@code true}；也就是说，当一个bean定义被完全解析时，将触发一个事件。覆盖此内容以返回{@code false}，以抑制该事件。
	 *
	 ...
	 * {@code true}，以便在解析bean定义后触发组件注册事件； {@code false}禁止事件
