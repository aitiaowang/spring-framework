...
public abstract class AbstractEnvironment implements ConfigurableEnvironment {
	 ...
	 * <p>
	 * 请注意，某些shell环境（例如Bash）不允许在变量名称中使用句点字符。假设正在使用Spring的{@link SystemEnvironmentPropertySource}，
	 * 则可以将此属性指定为环境变量，例如{@code SPRING_PROFILES_DEFAULT}。
	 *
	 ...
	 * <p>
	 * 返回通过{@link #setDefaultProfiles (String...)}明确设置的默认配置文件集，
	 * 或者如果当前的默认配置文件集仅由{@linkplain #getReservedDefaultProfiles() 保留的默认配置文件}组成，
	 * 然后检查{@value #DEFAULT_PROFILES_PROPERTY_NAME}属性的存在，并将其值（如果有）*分配给默认配置文件集。
	 *
