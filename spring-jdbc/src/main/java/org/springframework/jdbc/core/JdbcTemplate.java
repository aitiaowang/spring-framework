 ...
 * <p>
 * 这是JDBC核心软件包中的中心类。它简化了JDBC的使用并有助于避免常见错误。
 * 它执行核心JDBC工作流程，而留下应用程序代码以提供SQL并提取结果。
 * 此类执行SQL查询或更新，在ResultSet上启动迭代，并捕获JDBC异常，
 * 并将其转换为{@code org.springframework.dao}包中定义的通用，信息量更大的异常层次结构。
 *
...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
	...
	public <T> T execute(StatementCallback<T> action) throws DataAccessException {
		...
		try {
			...
			// Release Connection early, to avoid potential connection pool deadlock
			// in the case when the exception translator hasn't been initialized yet.
			// 尽早释放Connection，以避免潜在的连接池死锁在尚未初始化异常转换器的情况下。
			String sql = getSql(action);
	...
	public <T> T execute(PreparedStatementCreator psc, PreparedStatementCallback<T> action)
		...
		try {
			...
			// Release Connection early, to avoid potential connection pool deadlock
			// in the case when the exception translator hasn't been initialized yet.
			// 尽早释放Connection，以避免在尚未初始化异常转换器的情况下潜在的连接池死锁。
	 ...
	 * <p>
	 * 使用准备好的语句进行查询，允许使用PreparedStatementCreator 和PreparedStatementSetter。
	 * 大多数其他查询方法都使用此方法，但应用程序代码将始终与创建者或设置者一起使用。
	 *
	 ...
	 *
	 * @param requiredType the type that each result object is expected to match  每个结果对象应匹配的类型
	 * @return the RowMapper to use
	 ...
	 * <p>
	 * 使用传入的args和类型创建一个新的基于arg类型的PreparedStatementSetter。
	 *
	 * <p>By default, we'll create an {@link ArgumentTypePreparedStatementSetter}.
	 * This method allows for the creation to be overridden by subclasses.
	 * <p>
	 * 默认情况下，我们将创建一个{@link ArgumentTypePreparedStatementSetter}。此方法允许创建被子类覆盖。
	 *
	 * @param args     object array with arguments
