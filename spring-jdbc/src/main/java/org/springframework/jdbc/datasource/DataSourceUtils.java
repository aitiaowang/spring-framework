...
public abstract class DataSourceUtils {
	 ...
	 * <p>
	 * 从给定的数据源获取一个连接。将SQLExceptions转换为未经检查的通用数据访问异常的Spring层次结构，从而简化了调用代码并使所有引发的异常都更有意义。
	 *
	 ...
	 * <p>
	 * 知道绑定到当前线程的对应连接，例如，在使用{@link DataSourceTransactionManager}时。如果事务同步处于活动状态，
	 * 例如，将Connection连接到线程。在{@link org.springframework.transaction.jta.JtaTransactionManager JTA}事务中运行时）。
	 *
	 * @param dataSource the DataSource to obtain Connections from  数据源以从中获取连接
	 ...
	 * 如果事务同步处于活动状态（例如，在JTA事务中），则将Connection连接绑定到线程。由{@link TransactionAwareDataSourceProxy}直接访问。
	 * <p>
	...
	public static Connection doGetConnection(DataSource dataSource) throws SQLException {
		...
		// 否则，我们要么没有支架，要么没有空的线程绑定支架。
		logger.debug("Fetching JDBC Connection from DataSource ==> 从数据源获取JDBC连接");
		...
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			try {
				...
				//对事务内的进一步JDBC操作使用相同的Connection。 线程绑定对象将在事务完成时通过同步删除。
				// 在事务中使用同一数据库连接
	 ...
	 * <p>
	 * 如果不是从外部管理（即未绑定到线程），则关闭从给定的DataSource获得的给定的Connection
	 *
	 ...
	 * <p>
	 * 实际上关闭从给定DataSource获得的给定Connection。与{@link #releaseConnection}相同，但会引发原始SQLException。
	 *
	 * <p>Directly accessed by {@link TransactionAwareDataSourceProxy}.
	 * <p>
	 ...
	 *
	 * @param con        the Connection to close if necessary
	 *                   (if this is {@code null}, the call will be ignored)  需要时关闭连接（如果为{@code null}，则将忽略该调用）
	 * @param dataSource the DataSource that the Connection was obtained from
	 *                   (may be {@code null})   从其中获取连接的数据源(可能是{@code null})
	 * @throws SQLException if thrown by JDBC methods
	 * @see #doGetConnection
	 */
	public static void doReleaseConnection(@Nullable Connection con, @Nullable DataSource dataSource) throws SQLException {
		...
		if (dataSource != null) {
			ConnectionHolder conHolder = (ConnectionHolder) TransactionSynchronizationManager.getResource(dataSource);
			// 当前线程存在事务的情况下，说明存在共用数据库连接，直接使用ConnectionHolder中的released方法进行连接数减一而不是真正的释放连接
